...
const GROUND_CLASS_PREFIX = 'ground-';
...
function viewportStageBox() {
  const chrome = dialogEl?.querySelector('.image-lightbox__chrome');
  const chromeH = chrome?.getBoundingClientRect().height ?? 0;
  return {
    maxW: Math.max(window.innerWidth * 0.96, 1),
    maxH: Math.max(window.innerHeight * 0.96 - chromeH, 1)
  };
}
...
function computeFitSize() {
  ...
  const { maxW, maxH } = viewportStageBox();
...
function resolveProseImage(target) {
  ...
  const wrap = target.closest('.halftone, .prose-img-iso__frame');
  if (wrap) {
 ...
 * @returns {string}
 */
export function resolveLightboxCaption(img) {
...
/**
 * Nearest poster ground for lightbox theming.
 * @param {Element} source
 * @returns {string | null}
 */
export function resolveLightboxGround(source) {
  const host = source.closest('.post-card[class*="ground-"]');
  if (!host) return null;
  return [...host.classList].find((c) => c.startsWith(GROUND_CLASS_PREFIX)) ?? null;
}

/** @param {HTMLElement} el */
function clearGroundClasses(el) {
  for (const cls of el.classList) {
    if (cls.startsWith(GROUND_CLASS_PREFIX)) el.classList.remove(cls);
  }
}

/** @param {string | null} groundClass */
function applyLightboxGround(source) {
  const shell = dialogEl?.querySelector('.image-lightbox__shell');
  if (shell instanceof HTMLElement) {
    clearGroundClasses(shell);
    const groundClass = resolveLightboxGround(source);
    if (groundClass) shell.classList.add(groundClass);
  }

  if (!dialogEl) return;
  dialogEl.style.removeProperty('--lightbox-backdrop-color');
  const card = source.closest('.post-card[class*="ground-"]');
  if (card instanceof HTMLElement) {
    const ink = getComputedStyle(card).getPropertyValue('--on-ground-body').trim();
    if (ink) dialogEl.style.setProperty('--lightbox-backdrop-color', ink);
  }
}

/** @param {HTMLElement} el */
function resetLightboxTheme(el) {
  clearGroundClasses(el);
  el.style.removeProperty('--lightbox-backdrop-color');
}
...
async function openLightbox(source) {
  ...
  applyLightboxGround(source);

  const caption = resolveLightboxCaption(source);
  lightboxImg.src = src;
  lightboxImg.alt = source.getAttribute('alt')?.trim() || '';
...
export function closeLightbox() {
  ...
  if (dialogEl) {
    const shell = dialogEl.querySelector('.image-lightbox__shell');
    if (shell instanceof HTMLElement) clearGroundClasses(shell);
    resetLightboxTheme(dialogEl);
  }
  if (lightboxImg) {
...
export function setupImageLightbox(root, opts = {}) {
  ...
  root.querySelectorAll('.prose img').forEach((img) => {
    ...
    const label = resolveLightboxCaption(img);
  ...
  root.querySelectorAll('.prose .halftone').forEach((wrap) => {
    ...
    if (!wrap.hasAttribute('tabindex')) {
      ...
      const img = wrap.querySelector('img');
      const label = img instanceof HTMLImageElement ? resolveLightboxCaption(img) : '';
      wrap.setAttribute(
        'aria-label',
        label ? `View larger: ${label}` : 'View larger image'
      );
  ...
  root.querySelectorAll('.prose .prose-img-iso__frame').forEach((frame) => {
    frame.classList.add('prose-img--zoomable-wrap');
