...
export function buildLabGalleryConfig(base, state) {
  ...
  g.heroGlyph = {
    ...
    text: {
      ...
      glyphColor: state.heroGlyphColor
    },
    layout: {
...
export function readLabStateFromConfig(base) {
  ...
  const heroBlend = hero.blend ?? {};
  const heroAppearance = hero.appearance ?? {};
  const glyph = base.theme?.graphics?.glyph ?? {};
  const ht = base.theme?.graphics?.imageHalftone ?? {};

  const patternBlend = blend.modes?.[0] ?? 'overlay';
  const heroBlendMode = heroBlend.modes?.[0] ?? 'difference';
  const patternOpEntry = blend.opacity?.[patternBlend];
  const heroOpEntry = heroBlend.opacity?.[heroBlendMode];
  ...
  return {
    ...
    heroGlyphColor: heroText.glyphColor ?? 'display',
    ...
    heroOpacity:
      heroOpEntry?.min ?? heroOpEntry?.max ?? heroAppearance.opacityMin ?? heroAppearance.opacityMax ?? 0.25,
