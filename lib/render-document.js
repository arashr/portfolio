...
export function renderDocument(doc, fileLabel, opts = {}) {
  ...
  const postersHtml = doc.posters
    .map((poster) => {
      const ground =
        groundClassFromToken(poster.groundKey) ||
        groundForSlug(poster.slug, previousGround, recentGrounds);
      ...
      const glyphSymbolAttr = poster.glyphSymbol
        ? ` data-glyph-symbol="${escapeHtml(poster.glyphSymbol)}"`
        : '';

      return `
    <div class="post-card-wrap ${ground}">${bleedHtml}
      <article class="post-card ${ground} title-face-${face.id}" id="${escapeHtml(poster.slug)}" data-slug="${escapeHtml(poster.slug)}" data-title-chars="${(poster.plainTitle || '').length}"${glyphSymbolAttr}>
