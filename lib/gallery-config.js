...
export function codeChipBgMixExpr(surfaceVar = 'var(--surface)', liftPct = 20) {
  ...
  return `color-mix(in oklch, ${surfaceVar} ${towardSurface}%, var(--paper))`;
}
...
export function buildExportRootVars(cfg = activeConfig) {
  ...
  return (
    ...
    `--color-code-text:${resolveColor(code.text ?? 'paper', cfg)};` +
    `--code-inline-mix:${code.inlineSurfaceMix ?? '35%'};` +
...
export function applyGalleryConfigToDocument(cfg = activeConfig) {
  ...
  root.style.setProperty('--color-ink-soft', colors.inkSoft);
  root.style.setProperty('--color-ink-muted', colors.inkMute);
  root.style.setProperty('--color-accent', colors.red);
  root.style.setProperty('--color-accent-bright', colors.redBright);

  root.style.setProperty('--measure', layout.measure);
  root.style.setProperty('--poster-width', layout.posterWidth || layout.measure);
  const edgeMix = Math.min(0.4, Math.max(0.04, Number(layout.edgeStepMix) || 0.12));
  root.style.setProperty('--color-edge-mix', `${Math.round(edgeMix * 100)}%`);
  root.style.setProperty('--space-page-inline', layout.pad);
  root.style.setProperty('--space-scroll-offset', layout.scrollOffset);

  root.style.setProperty('--hero-display', resolveColor(hero.display, cfg));
  root.style.setProperty('--hero-body', resolveColor(hero.body, cfg));
  root.style.setProperty('--hero-muted', resolveColor(hero.muted, cfg));

  root.style.setProperty('--font-size-body', typo.bodySize);
  root.style.setProperty('--line-height-body', String(typo.bodyLineHeight));
  root.style.setProperty('--line-height-prose', String(typo.proseLineHeight ?? '1.6'));
  root.style.setProperty('--line-height-title', String(typo.titleLineHeight ?? '1.12'));
  ...
  root.style.setProperty('--font-size-label', typo.labelSize);
  root.style.setProperty('--font-weight-label', String(typo.labelWeight));
  root.style.setProperty('--letter-spacing-label', typo.labelLetterSpacing);
  root.style.setProperty('--font-size-prose', typo.proseSize);
  root.style.setProperty('--font-size-crumb', typo.crumbSize);

  root.style.setProperty('--card-hover-ease', motion.cardHoverEase);
  root.style.setProperty('--card-hover-duration', motion.cardHoverDuration);
  root.style.setProperty('--reveal-duration', motion.revealDuration ?? '0.8s');
  root.style.setProperty('--reveal-ease', motion.revealEase ?? 'ease');
  root.style.setProperty('--shutter-duration', motion.shutterDuration ?? '0.9s');
  root.style.setProperty('--shutter-ease', motion.shutterEase ?? 'cubic-bezier(0.77, 0, 0.18, 1)');
  ...
  root.style.setProperty('--glyph-pattern-opacity', String(glyphOpacity));
  ...
  root.style.setProperty('--color-code-text', resolveColor(code.text ?? 'paper', cfg));
  root.style.setProperty('--code-inline-mix', code.inlineSurfaceMix ?? '35%');
  const blockMix = resolveCodeStepMix(cfg);
  root.style.setProperty('--code-step-mix', `${Math.round(blockMix * 100)}%`);

  root.style.setProperty('--color-dark-paper', dark.paper ?? DEFAULT_CONFIG.darkTheme.paper);
  root.style.setProperty('--color-dark-ink', darkColors.ink);
  root.style.setProperty('--color-dark-ink-soft', darkColors.inkSoft);
  root.style.setProperty('--color-dark-ink-muted', darkColors.inkMute);
  root.style.setProperty('--color-dark-accent', darkColors.red);
  root.style.setProperty('--color-dark-accent-bright', darkColors.redBright);
  root.style.setProperty(
    '--dark-drop-butter-mix',
