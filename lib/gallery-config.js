...
import { resolveColorPool } from './glyph-colors.js';
import {
...
function layoutTokens(cfg = activeConfig) {
  const layout = { ...DEFAULT_CONFIG.theme.layout, ...(cfg.theme?.layout || {}) };
  return {
    measure: layout.measure ?? LAYOUT_TOKEN_DEFAULTS.measure,
    posterWidth: layout.posterWidth ?? LAYOUT_TOKEN_DEFAULTS.posterWidth,
    edgeStepMix:
      layout.edgeStepMix != null
        ? clampMix(Number(layout.edgeStepMix))
        : parseEdgeMixPercent(readRootToken('--color-edge-mix', '12%')),
    pad: layout.pad ?? readRootToken('--space-page-inline', LAYOUT_TOKEN_DEFAULTS.pad),
    scrollOffset:
      layout.scrollOffset ?? readRootToken('--space-scroll-offset', LAYOUT_TOKEN_DEFAULTS.scrollOffset),
    layoutMax: layout.layoutMax
  };
}
...
/** @param {{ enabled?: boolean, columns?: boolean, rows?: boolean, bounds?: boolean } | undefined} overlay */
export function portfolioGridOverlayMode(overlay) {
  if (!overlay?.enabled) return '';
  const columns = overlay.columns !== false;
  const rows = overlay.rows !== false;
  if (columns && rows) return 'both';
  if (columns) return 'columns';
  if (rows) return 'rows';
  return '';
}

function resolveGridOverlay(cfg = activeConfig) {
  const overlay = {
    ...DEFAULT_CONFIG.theme.gridOverlay,
    ...(cfg.theme?.gridOverlay || {})
  };
  return {
    enabled: overlay.enabled === true,
    columns: overlay.columns !== false,
    rows: overlay.rows !== false,
    bounds: overlay.bounds === true
  };
}

export function resolveRowSnap(cfg = activeConfig) {
  const rowSnap = {
    ...DEFAULT_CONFIG.theme.rowSnap,
    ...(cfg.theme?.rowSnap || {})
  };
  const packAlign =
    rowSnap.packAlign === 'start' || rowSnap.packAlign === 'center'
      ? rowSnap.packAlign
      : 'vary';
  return {
    enabled: rowSnap.enabled === true,
    packAlign,
    beats: Array.isArray(rowSnap.beats) ? rowSnap.beats : undefined
  };
}

export function resolveTitlePlay(cfg = activeConfig) {
  const titlePlay = {
    ...DEFAULT_CONFIG.theme.titlePlay,
    ...(cfg.theme?.titlePlay || {})
  };
  return {
    enabled: titlePlay.enabled !== false,
    fillSafeZone: titlePlay.fillSafeZone !== false,
    maxScale: Number.isFinite(Number(titlePlay.maxScale))
      ? Math.max(1, Math.min(2, Number(titlePlay.maxScale)))
      : 1.35,
    shortTitleChars: Number.isFinite(Number(titlePlay.shortTitleChars))
      ? Number(titlePlay.shortTitleChars)
      : 22,
    shortTitleBoost: Number.isFinite(Number(titlePlay.shortTitleBoost))
      ? Math.max(1, Math.min(1.6, Number(titlePlay.shortTitleBoost)))
      : 1.15
  };
}
...
const DEFAULT_CONFIG = {
  theme: {
    ...
    layout: {
      measure: LAYOUT_TOKEN_DEFAULTS.measure,
      posterWidth: LAYOUT_TOKEN_DEFAULTS.posterWidth,
      edgeStepMix: LAYOUT_TOKEN_DEFAULTS.edgeStepMix,
      pad: LAYOUT_TOKEN_DEFAULTS.pad,
      scrollOffset: LAYOUT_TOKEN_DEFAULTS.scrollOffset
    },
    graphics: {
...
/** @param {GroundDef} def @param {{ color: string, colors?: string[], opacity: number }} globalGlyph */
export function resolveGroundGlyphTokens(def, cfg, globalGlyph) {
  const groundGlyph = def.glyph || {};
  const pool = resolveColorPool(groundGlyph, resolveColorPool(globalGlyph));
  const colorKey = pool[0] || globalGlyph.color;
  const color =
    colorKey === 'display'
      ? resolveColor(def.foreground?.display, cfg)
      : resolveColor(colorKey, cfg);

  return { color, colors: pool.length ? pool : [color], opacity: 1 };
}
...
/**
 * Landing `#landing-name` hero — flat scale only (no length tiers).
 * @param {import('./gallery-config.js').GalleryConfig} [cfg]
 */
export function resolveLandingHeaderConfig(cfg = activeConfig) {
  const defaults = DEFAULT_CONFIG.landing?.header ?? {};
  const patch = cfg.landing?.header || {};
  return {
    ...defaults,
    ...patch,
    titleScale: {
...
export function resolveLandingNameTitleScale(cfg = activeConfig) {
  const landing = { ...resolveLandingHeaderConfig(cfg).titleScale };
  if (!Array.isArray(landing.tiers)) landing.tiers = [];
  return landing;
}

function buildLandingHeaderStylesheet(cfg = activeConfig) {
  const header = resolveLandingHeaderConfig(cfg);
  const layout = header.layout || {};
  const titleSpan = Math.max(1, Math.min(12, Number(layout.titleColumnSpan) || 9));
  const copySpan = Math.max(1, Math.min(12, Number(layout.copyColumnSpan) || 5));
  const minHeight =
    typeof layout.minHeight === 'string' && layout.minHeight.trim()
      ? layout.minHeight.trim()
      : '66vh';
  const lines = [
    ':root {',
    `  --landing-header-title-columns: 1 / span ${titleSpan};`,
    `  --landing-header-copy-columns: 1 / span ${copySpan};`,
    `  --landing-header-min-height: ${minHeight};`,
    '}'
  ];
  const glyphColors = resolveColorPool(header.glyph || {});
  if (glyphColors.length) {
    const resolved = resolveColor(glyphColors[0], cfg);
    lines.push(
      `.page-landing #landing-name > .post-card.landing-name-card {`,
      `  --glyph-pattern-color: ${resolved};`,
      `  --glyph-pattern-opacity: 1;`,
      '}'
    );
  }
  return lines.join('\n');
...
function ensureLandingHeaderStyleEl() {
  let el = document.getElementById('gallery-config-landing-header');
  if (!el) {
    el = document.createElement('style');
    el.id = 'gallery-config-landing-header';
...
export function applyGalleryConfigToDocument(cfg = activeConfig) {
  ...
  const typo = typographyTokens();
  const layout = layoutTokens(cfg);
  root.style.setProperty('--measure', layout.measure);
  root.style.setProperty('--poster-width', layout.posterWidth);
  root.style.setProperty('--space-page-inline', layout.pad);
  root.style.setProperty('--space-scroll-offset', layout.scrollOffset);
  if (layout.layoutMax) root.style.setProperty('--layout-max', layout.layoutMax);
  root.style.setProperty(
    '--line-height-ui-sans',
  ...
  const gridOverlay = resolveGridOverlay(cfg);
  const overlayMode = portfolioGridOverlayMode(gridOverlay);
  if (overlayMode) {
    root.dataset.portfolioGridOverlay = overlayMode;
  } else {
    delete root.dataset.portfolioGridOverlay;
  }
  if (gridOverlay.enabled && gridOverlay.bounds) {
    root.dataset.portfolioGridBounds = 'true';
  } else {
    delete root.dataset.portfolioGridBounds;
  }
  ...
  if (typeof document !== 'undefined') {
    ...
    ensureLandingHeaderStyleEl().textContent = buildLandingHeaderStylesheet(cfg);
  }
}
