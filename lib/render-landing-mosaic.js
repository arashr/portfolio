...
import { groundClassFromToken, groundForSlug } from './grounds.js';
import { titleFaceForIndex } from './title-faces.js';
import { getGalleryConfig, resolveLandingHeaderConfig } from './gallery-config.js';
import {
  pickBleedSymbol,
...
/** @typedef {{ path: string, title: string, index: number, subtext?: string, stats?: { value: string, label: string }[], credit?: string, coverGroundKey?: string | null }} CaseItem */
 ...
 * @param {{ previousGround: string | null, recentGrounds: string[], previousFaceId: string | null, previousTitleBleed: TitleBleedState }} state
 * @param {string} symbolPool
 */
function renderNamePoster(site, state, symbolPool) {
  const header = resolveLandingHeaderConfig(getGalleryConfig());
  const ground = groundClassFromToken(header.ground) || 'ground-indigo';
  const faceId = header.titleFace || 'bricolage-grotesque';
  ...
  state.recentGrounds = [...state.recentGrounds, ground].slice(-2);
  state.previousFaceId = faceId;
  setNextTitleBleed(state, plainTitle, ground, symbolPool);

  const glyphAttrs = [];
  if (header.glyph && Object.keys(header.glyph).length) {
    glyphAttrs.push('data-lab-glyph="pattern"');
    const forced = header.glyph.symbol?.trim();
    if (forced) glyphAttrs.push(`data-glyph-symbol="${escapeHtml(forced)}"`);
  }

  return `<div id="landing-name" class="post-card-wrap ${ground} landing-name-wrap" role="presentation">
    <article class="post-card ${ground} title-face-${faceId} landing-name-card" data-slug="landing-name" data-title-chars="${plainTitle.length}"${glyphAttrs.length ? ` ${glyphAttrs.join(' ')}` : ''}>
...
function renderCasePoster(item, state, symbolPool) {
  const { path, title, index, subtext, stats = [], credit = '', coverGroundKey = null } = item;
  const slug = slugify(path.replace(/\.[^.]+$/, '')) || 'file';
  // A5-COVER-GROUND: explicit frontmatter ground only; no symbol/glyph wiring.
  const ground =
    groundClassFromToken(coverGroundKey) ||
    groundForSlug(slug, state.previousGround, state.recentGrounds);
...
function renderAsidePoster(section, index, state, symbolPool) {
  ...
  const ground = groundForSlug(slug, state.previousGround, state.recentGrounds);
