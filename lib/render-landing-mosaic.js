...
import { groundClassFromToken, groundForSlug, resolveSequencedGround } from './grounds.js';
...
function renderCasePoster(item, state, symbolPool) {
  ...
  // Cover frontmatter color when it does not collide with recent grounds.
  const ground = resolveSequencedGround(
    slug,
    state.previousGround,
    state.recentGrounds,
    coverGroundKey
  );
