...
import { normalizeGroundKey } from './grounds.js';
...
/**
 * First non-digit, non-whitespace character for glyph patterns.
 * @param {unknown} value
 * @returns {string | null}
 */
export function firstGlyphSymbol(value) {
  if (value == null) return null;
  for (const ch of String(value)) {
    if (ch.trim() && !/[0-9]/.test(ch)) return ch;
  }
  return null;
}

/**
 * Optional first-poster (cover) overrides from YAML frontmatter.
 * @param {Record<string, string>} meta
 * @returns {{ groundKey: string | null, glyphSymbol: string | null }}
 */
export function resolveCoverOverrides(meta = {}) {
  const groundKey = normalizeGroundKey(meta.ground);
  const glyphSymbol = firstGlyphSymbol(meta.symbol ?? meta.patternSymbol);
  return { groundKey, glyphSymbol };
}
...
export function peekCaseStudyListing(rawText, filename = 'document.md') {
  ...
  // A5-COVER-GROUND: optional first-poster ground for home / more-cases cards (color only).
  const coverGroundKey = resolveCoverOverrides(meta).groundKey;
...
export function parseDocument(rawText, filename = 'document.md') {
  ...
  const cover = resolveCoverOverrides(meta);
  if (cover.groundKey || cover.glyphSymbol) {
    posters = posters.map((poster, index) => {
      if (index !== 0) return poster;
      return {
        ...poster,
        ...(cover.groundKey ? { groundKey: cover.groundKey } : {}),
        ...(cover.glyphSymbol ? { glyphSymbol: cover.glyphSymbol } : {})
      };
    });
  }
