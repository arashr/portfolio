...
/**
 * @param {{ title?: string, ogTitle?: string, tagline?: string, description?: string, url?: string, ogImageWidth?: number, ogImageHeight?: number }} site
 */
export function buildSocialMetaTags(site = {}) {
  const title =
    site.ogTitle?.trim() || site.title?.trim() || 'Arash Ranjbaran | Portfolio';
  const description =
    site.description?.trim() || 'Case studies in product design, design systems, and growth.';
  const baseUrl = (site.url || 'https://arash.design/').replace(/\/?$/, '/');
