/**
 * Render poster glyph patterns / hero glyphs onto canvas layers.
 */

import { renderTypePattern } from './type-pattern.js';
import { computeHeroGlyphEmptyRegion, computePosterGlyphRegion } from './glyph-region.js';
import { resolveGlyphCanvasOverscan } from './glyph-canvas-overscan.js';
import {
  buildPosterTypePatternOptions,
  pickPosterSymbolLetter,
  posterTitleLetter
} from './type-pattern-poster.js';
import { pickGlyphColor } from './glyph-colors.js';
import {
  resolveCardHeroGlyphConfig,
  resolveCardPatternConfig
} from './resolve-ground-graphics.js';
import {
  applyHeroGlyph,
  clearHeroGlyph,
  isHeroGlyphAllowed,
  isHeroGlyphFaceAllowed,
  pickHeroGlyphText,
  posterRandFromSlug,
  refitHeroGlyph,
  resolveHeroGlyphConfig,
  shouldUseHeroGlyph
} from './poster-hero-glyph.js';

function clampNum(n, fallback) {
  const v = Number.parseFloat(String(n));
  return Number.isFinite(v) ? v : fallback;
}

/**
 * Forced `data-glyph-symbol` always draws a pattern letter and never rolls a hero / none.
 * @param {string} [forcedSymbol]
 * @param {string} [labForce]
 */
export function resolveForcedGlyphPlan(forcedSymbol = '', labForce = '') {
  const forced = Boolean(String(forcedSymbol || '').trim());
  return {
    forcePattern: forced,
    allowHero: !forced,
    allowNone: !forced && labForce !== 'pattern'
  };
}

/**
 * @param {HTMLElement[]} cards `.post-card` elements
 * @param {import('./gallery-config.js').GalleryConfig} cfg
 */
export function renderPosterGlyphPatterns(cards, cfg) {
  if (!cards?.length) return;
  const globalHeroCfg = resolveHeroGlyphConfig(cfg);
  const glyphRatio = Number.parseFloat(String(cfg?.theme?.scrollParallax?.glyphRatio));
  const glyphExtras = {
    glyphRatio: Number.isFinite(glyphRatio) ? glyphRatio : undefined
  };

  /** @type {string | null} */
  let previousRegionKey = null;

  for (const card of cards) {
    const patternCfg = resolveCardPatternConfig(cfg, card);
    const heroCfg = resolveCardHeroGlyphConfig(cfg, card);
    const canvas = card.querySelector('[data-glyph-canvas]');
    if (!(canvas instanceof HTMLCanvasElement)) continue;
    const layer = card.querySelector('.post-card__glyph-layer');
    if (!(layer instanceof HTMLElement)) continue;

    const title = (
      card.querySelector('.post-title')?.textContent ||
      card.querySelector('.mini-poster__title')?.textContent ||
      card.querySelector('.landing-mosaic__aside-title')?.textContent ||
      card.dataset.slug ||
      'A'
    ).trim();
    const titleLetter = posterTitleLetter(title);
    const titleEl = card.querySelector(
      '.post-title a, .post-title, .mini-poster__title, .landing-mosaic__aside-title'
    );
    const cs = getComputedStyle(card);
    const titleStyle = titleEl ? getComputedStyle(titleEl) : cs;
    const rand = posterRandFromSlug(card.dataset.slug || title);
    const pick = (arr) => arr[Math.floor(rand() * arr.length)];
    const int = (min, max) => Math.floor(rand() * (max - min + 1)) + min;
    const slug = card.dataset.slug || title;
    const patternColorRand = posterRandFromSlug(`${slug}:pattern-color`);
    const heroColorRand = posterRandFromSlug(`${slug}:hero-color`);
    const patternColor = pickGlyphColor(
      patternCfg,
      patternColorRand,
      cs.getPropertyValue('--glyph-pattern-color').trim() || '#c8102e'
    );

    const labForce = card.dataset.labGlyph;
    const forcedSymbol = card.dataset.glyphSymbol?.trim() || '';
    const forcedPlan = resolveForcedGlyphPlan(forcedSymbol, labForce);
    const useHero =
      forcedPlan.allowHero &&
      (labForce === 'hero' ||
        (labForce !== 'pattern' &&
          shouldUseHeroGlyph(globalHeroCfg, rand) &&
          isHeroGlyphAllowed(globalHeroCfg) &&
          isHeroGlyphFaceAllowed(globalHeroCfg, card)));

    if (useHero) {
      const emptyRegion = patternCfg.preferEmptySpace
        ? computeHeroGlyphEmptyRegion(card, patternCfg, { slug })
        : null;
      applyHeroGlyph(
        card,
        layer,
        canvas,
        heroCfg,
        pickHeroGlyphText(heroCfg, patternCfg, title, rand),
        rand,
        heroColorRand,
        emptyRegion,
        glyphExtras
      );
      continue;
    }

    clearHeroGlyph(card, layer);

    const noneProb = Math.min(1, Math.max(0, clampNum(patternCfg.noneProbability, 0.18)));
    if (forcedPlan.allowNone && rand() < noneProb) {
      const ctx = canvas.getContext('2d');
      ctx?.clearRect(0, 0, canvas.width, canvas.height);
      canvas.style.display = 'none';
      continue;
    }

    canvas.style.display = 'block';
    canvas.style.opacity = '1';
    layer.dataset.glyphColor = patternColor;
    const patternLetter = forcedPlan.forcePattern
      ? forcedSymbol
      : pickPosterSymbolLetter(patternCfg, titleLetter, rand, pick);
    const region = computePosterGlyphRegion(card, patternCfg, { slug, previousRegionKey, rand });
    previousRegionKey = region.regionKey;
    const glyphW = Math.max(1, Math.round(region.width));
    const glyphH = Math.max(1, Math.round(region.height));
    const { padX, padY } = resolveGlyphCanvasOverscan(glyphW, glyphH, glyphExtras);
    const drawW = glyphW + 2 * padX;
    const drawH = glyphH + 2 * padY;
    layer.style.setProperty('--glyph-x', `${Math.round(region.x - padX)}px`);
    layer.style.setProperty('--glyph-y', `${Math.round(region.y - padY)}px`);
    layer.style.setProperty('--glyph-w', `${drawW}px`);
    layer.style.setProperty('--glyph-h', `${drawH}px`);
    renderTypePattern(canvas, {
      ...buildPosterTypePatternOptions(patternCfg, {
        rand,
        pick,
        int,
        float: (min, max) => min + rand() * (max - min),
        letter: patternLetter,
        foregroundColor: patternColor,
        fontFamily: titleStyle.fontFamily,
        fontWeight: titleStyle.fontWeight,
        width: drawW,
        height: drawH
      }),
      blendMode: null,
      surfaceColor: null,
      backgroundColor: 'rgba(0,0,0,0)'
    });
  }

  for (const card of cards) {
    refitHeroGlyph(
      card,
      resolveCardHeroGlyphConfig(cfg, card),
      (c, slug) => {
        const placementCfg = resolveCardPatternConfig(cfg, c);
        return placementCfg.preferEmptySpace
          ? computeHeroGlyphEmptyRegion(c, placementCfg, { slug })
          : null;
      },
      glyphExtras
    );
  }
}
