...
import { resolveSequencedGround } from './grounds.js';
...
export function renderMiniPoster(opts) {
  ...
  const ground = resolveSequencedGround(slug, previousGround, recentGrounds, groundKey);
...
export function renderMiniPosterGrid(items) {
  ...
  for (const { path, title, subtext, stats, credit, index, coverGroundKey = null } of items) {
    ...
    const ground = resolveSequencedGround(slug, previousGround, recentGrounds, coverGroundKey);
    const face = titleFaceForIndex(index, previousFaceId);
