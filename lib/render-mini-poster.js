 ...
 *   extraClass?: string,
 *   previousGround?: string | null,
 *   previousFaceId?: string | null,
 *   recentGrounds?: string[],
...
export function renderMiniPoster(opts) {
  const {
    ...
    previousFaceId = null,
    recentGrounds = [],
    // A5-COVER-GROUND: color override only (no data-glyph-symbol).
    groundKey = null
  } = opts;
  const slug = slugIn || slugify(title) || 'poster';
  const ground =
    groundClassFromToken(groundKey) || groundForSlug(slug, previousGround, recentGrounds);
...
/** @param {{ path: string, title: string, subtext?: string, stats?: { value: string, label: string }[], credit?: string, index: number, coverGroundKey?: string | null }[]} items */
export function renderMiniPosterGrid(items) {
  let previousGround = null;
  let recentGrounds = [];
  let previousFaceId = null;
  const parts = [];
  for (const { path, title, subtext, stats, credit, index, coverGroundKey = null } of items) {
    const slug = slugify(path.replace(/\.[^.]+$/, '')) || 'file';
    // A5-COVER-GROUND: color override only.
    const ground =
      groundClassFromToken(coverGroundKey) ||
      groundForSlug(slug, previousGround, recentGrounds);
    const face = titleFaceForIndex(index, previousFaceId);
    const poster = renderMiniPoster({
      ...
      previousFaceId,
      recentGrounds,
      groundKey: coverGroundKey
    });
    parts.push(
      `<div class="post-card-wrap ${ground} landing-pick-wrap">${poster}</div>`
    );
    previousGround = ground;
    recentGrounds = [...recentGrounds, ground].slice(-2);
