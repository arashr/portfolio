/**
 * Huge background glyph posters — alternative to mini type patterns.
 * Config: `theme.graphics.heroGlyph` (see HERO_GLYPH_DEFAULTS).
 * Symbol source: `theme.graphics.typePattern.symbolPool` + `symbolProbability`.
 * Fill color: solid `colors[]` pool (slug-seeded random pick).
 */

import { pickPosterSymbolLetter, posterTitleLetter } from './type-pattern-poster.js';
import { pickGlyphColor, resolveColorPool } from './glyph-colors.js';
import { flattenHeroGlyphConfig } from './resolve-graphics-config.js';
import {
  expandOverscanForInk,
  resolveGlyphCanvasOverscan
} from './glyph-canvas-overscan.js';

export const HERO_GLYPH_DEFAULTS = {
  probability: 0.22,
  lengthMin: 1,
  lengthMax: 2,
  colors: ['#c8102e'],
  color: '#c8102e',
  sizeRatio: 0.98,
  minVisibleRatio: 0.8,
  /** Title face ids (`fonts.titleFaces[].id`) that never get a mega-glyph. */
  excludeTitleFaces: [],
  /** Horizontal shift as fraction of card width; negative = left, positive = right. */
  offsetXRatioMin: -0.28,
  offsetXRatioMax: 0.28,
  /** Skip mega-glyph when OS asks for reduced transparency (DESIGN.md). */
  respectReducedTransparency: true,
  /** Skip mega-glyph when OS asks for higher contrast. */
  respectHighContrast: true
};

/** @param {string} slug */
export function posterRandFromSlug(slug) {
  const seedBase = String(slug || 'poster')
    .split('')
    .reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
  let s = seedBase >>> 0;
  return () => {
    s = (s * 1664525 + 1013904223) >>> 0;
    return s / 4294967296;
  };
}

/** @param {import('./gallery-config.js').GalleryConfig | undefined} cfg */
export function resolveHeroGlyphConfig(cfg) {
  const flat = flattenHeroGlyphConfig(cfg);
  const merged = { ...HERO_GLYPH_DEFAULTS, ...flat };
  const pool = resolveColorPool(merged, HERO_GLYPH_DEFAULTS.colors);
  merged.colors = pool;
  merged.color = pool[0] || HERO_GLYPH_DEFAULTS.color;
  return merged;
}

/** @param {import('./gallery-config.js').GalleryConfig | undefined} cfg */
export function resolveHeroGlyphOptions(cfg) {
  const nested = resolveHeroGlyphConfig(cfg);
  const lengthMin = clampInt(nested.lengthMin, HERO_GLYPH_DEFAULTS.lengthMin, 1, 6);
  const lengthMax = clampInt(nested.lengthMax, HERO_GLYPH_DEFAULTS.lengthMax, lengthMin, 8);
  const pool = resolveColorPool(nested, HERO_GLYPH_DEFAULTS.colors);

  return {
    probability: clampNum(nested.probability, HERO_GLYPH_DEFAULTS.probability, 0, 1),
    lengthMin,
    lengthMax,
    colors: pool,
    color: pool[0] || HERO_GLYPH_DEFAULTS.color,
    sizeRatio: clampNum(nested.sizeRatio, HERO_GLYPH_DEFAULTS.sizeRatio, 0.4, 1.2),
    minVisibleRatio: clampNum(
      nested.minVisibleRatio,
      HERO_GLYPH_DEFAULTS.minVisibleRatio,
      0,
      1
    ),
    excludeTitleFaces: Array.isArray(nested.excludeTitleFaces)
      ? nested.excludeTitleFaces.map((id) => String(id))
      : [...HERO_GLYPH_DEFAULTS.excludeTitleFaces],
    offsetXRatioMin: clampNum(
      nested.offsetXRatioMin,
      HERO_GLYPH_DEFAULTS.offsetXRatioMin,
      -1.5,
      1.5
    ),
    offsetXRatioMax: clampNum(
      nested.offsetXRatioMax,
      HERO_GLYPH_DEFAULTS.offsetXRatioMax,
      -1.5,
      1.5
    ),
    respectReducedTransparency: nested.respectReducedTransparency !== false,
    respectHighContrast: nested.respectHighContrast !== false
  };
}

/**
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} opts
 * @param {() => number} rand
 */
export function shouldUseHeroGlyph(opts, rand) {
  return rand() < opts.probability;
}

/**
 * OS / readability gates — mega-glyphs are decorative; skip when the user asks for less visual noise.
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} opts
 * @param {{ matchMedia?: (q: string) => { matches: boolean } }} [env]
 */
export function isHeroGlyphAllowed(opts, env = typeof globalThis !== 'undefined' ? globalThis : undefined) {
  const mq = env?.matchMedia;
  if (typeof mq !== 'function') return true;
  if (opts.respectReducedTransparency !== false && mq('(prefers-reduced-transparency: reduce)').matches) {
    return false;
  }
  if (opts.respectHighContrast !== false && mq('(prefers-contrast: more)').matches) {
    return false;
  }
  return true;
}

/** @param {HTMLElement} card */
export function posterTitleFaceId(card) {
  if (!card) return null;
  const match = String(card.className).match(/\btitle-face-([\w-]+)\b/);
  return match?.[1] ?? null;
}

/**
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} opts
 * @param {HTMLElement} card
 */
export function isHeroGlyphFaceAllowed(opts, card) {
  const exclude = opts.excludeTitleFaces;
  if (!exclude?.length) return true;
  const faceId = posterTitleFaceId(card);
  if (!faceId) return true;
  return !exclude.includes(faceId);
}

/**
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} opts
 * @param {number} cardW
 * @param {() => number} rand
 */
export function pickHeroGlyphOffsetX(opts, cardW, rand) {
  const lo = Math.min(opts.offsetXRatioMin, opts.offsetXRatioMax);
  const hi = Math.max(opts.offsetXRatioMin, opts.offsetXRatioMax);
  const w = Math.max(1, cardW);
  return w * (lo + rand() * (hi - lo));
}

/**
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} heroOpts
 * @param {Record<string, unknown>} patternCfg
 * @param {string} title
 * @param {() => number} rand
 */
export function pickHeroGlyphText(heroOpts, patternCfg, title, rand) {
  const int = (min, max) => Math.floor(rand() * (max - min + 1)) + min;
  const pick = (arr) => arr[Math.floor(rand() * arr.length)];
  const titleLetter = posterTitleLetter(title);
  const len = int(heroOpts.lengthMin, heroOpts.lengthMax);

  let out = '';
  for (let i = 0; i < len; i++) {
    out += pickPosterSymbolLetter(patternCfg, titleLetter, rand, pick);
  }
  return out || titleLetter;
}

/**
 * @param {number} value
 * @param {number} fallback
 * @param {number} min
 * @param {number} max
 */
function clampNum(value, fallback, min, max) {
  const n = Number.parseFloat(String(value));
  const v = Number.isFinite(n) ? n : fallback;
  return Math.min(max, Math.max(min, v));
}

/**
 * @param {number} value
 * @param {number} fallback
 * @param {number} min
 * @param {number} max
 */
function clampInt(value, fallback, min, max) {
  const n = Number.parseInt(String(value), 10);
  const v = Number.isFinite(n) ? n : fallback;
  return Math.min(max, Math.max(min, v));
}

/**
 * Resolve a solid fill for hero glyphs from `colors[]` (or legacy semantic `color`).
 * @param {HTMLElement} card
 * @param {{ colors?: unknown, color?: unknown, glyphColor?: unknown }} opts
 * @param {() => number} [rand]
 */
export function resolveHeroGlyphPaint(card, opts, rand = () => 0) {
  const pool = resolveColorPool(opts);
  const semantic = new Set(['glyph', 'pattern', 'display', 'accent']);
  const solidPool = pool.filter((c) => !semantic.has(String(c).toLowerCase()));
  if (solidPool.length) {
    return pickGlyphColor({ colors: solidPool }, rand, solidPool[0]);
  }

  const key = String(opts.color ?? opts.glyphColor ?? pool[0] ?? '').toLowerCase();
  const cs = getComputedStyle(card);
  if (key === 'glyph' || key === 'pattern') {
    return cs.getPropertyValue('--glyph-pattern-color').trim() || '#111';
  }
  if (key === 'display') {
    return cs.getPropertyValue('--on-ground-display').trim() || '#111154';
  }
  if (key === 'accent') {
    return (
      cs.getPropertyValue('--on-ground-accent').trim() ||
      cs.getPropertyValue('--on-ground-display').trim() ||
      '#111'
    );
  }
  return (pool[0] || opts.color || '#c8102e');
}

/**
 * @param {CanvasRenderingContext2D} ctx
 * @param {number} sizePx
 * @param {CSSStyleDeclaration} titleStyle
 */
function applyHeroGlyphFont(ctx, sizePx, titleStyle) {
  const weight = titleStyle.fontWeight || '700';
  const family = titleStyle.fontFamily || 'serif';
  ctx.font = `${weight} ${sizePx}px ${family}`;
  const spacing = titleStyle.letterSpacing;
  if (spacing && spacing !== 'normal' && 'letterSpacing' in ctx) {
    ctx.letterSpacing = spacing;
  }
}

/**
 * @param {CanvasRenderingContext2D} ctx
 * @param {string} text
 * @param {number} targetW
 * @param {CSSStyleDeclaration} titleStyle
 * @param {number} maxHi
 */
export function fitHeroGlyphFontSize(ctx, text, targetW, titleStyle, maxHi) {
  let lo = 12;
  let hi = Math.max(24, maxHi);
  while (lo < hi) {
    const mid = Math.ceil((lo + hi) / 2);
    applyHeroGlyphFont(ctx, mid, titleStyle);
    const w = ctx.measureText(text).width;
    if (w > targetW) hi = mid - 1;
    else lo = mid;
  }
  return Math.max(12, lo);
}

/**
 * Keep the requested fraction of the glyph's measured ink bounds visible.
 * The square-root target on each axis guarantees the requested visible area.
 * @param {number} canvasW
 * @param {number} canvasH
 * @param {number} desiredX
 * @param {number} desiredY
 * @param {TextMetrics | Record<string, number>} metrics
 * @param {number} minVisibleRatio
 */
export function constrainHeroGlyphPlacement(
  canvasW,
  canvasH,
  desiredX,
  desiredY,
  metrics,
  minVisibleRatio
) {
  const ratio = clampNum(minVisibleRatio, HERO_GLYPH_DEFAULTS.minVisibleRatio, 0, 1);
  if (ratio <= 0) return { scale: 1, x: desiredX, y: desiredY };

  const axisRatio = Math.sqrt(ratio);
  const measuredWidth = Math.max(1, Number(metrics.width) || 1);
  const left = Math.max(0, Number(metrics.actualBoundingBoxLeft) || measuredWidth / 2);
  const right = Math.max(0, Number(metrics.actualBoundingBoxRight) || measuredWidth / 2);
  const ascent = Math.max(0, Number(metrics.actualBoundingBoxAscent) || measuredWidth / 2);
  const descent = Math.max(0, Number(metrics.actualBoundingBoxDescent) || 0);
  const inkW = Math.max(1, left + right);
  const inkH = Math.max(1, ascent + descent);
  const scale = Math.min(1, canvasW / (axisRatio * inkW), canvasH / (axisRatio * inkH));
  const scaledLeft = left * scale;
  const scaledRight = right * scale;
  const scaledAscent = ascent * scale;
  const scaledDescent = descent * scale;
  const clipX = (1 - axisRatio) * inkW * scale;
  const clipY = (1 - axisRatio) * inkH * scale;

  return {
    scale,
    x: Math.min(canvasW + clipX - scaledRight, Math.max(scaledLeft - clipX, desiredX)),
    y: Math.min(canvasH + clipY - scaledDescent, Math.max(scaledAscent - clipY, desiredY))
  };
}

/**
 * Apply oversized canvas box on the glyph layer so the section (layer overflow)
 * clips — not the bitmap edge.
 * @param {HTMLElement | null} layer
 * @param {number} padX
 * @param {number} padY
 * @param {number} viewW
 * @param {number} viewH
 */
function applyHeroCanvasBox(layer, padX, padY, viewW, viewH) {
  if (!(layer instanceof HTMLElement)) return;
  layer.style.setProperty('--glyph-x', `${-padX}px`);
  layer.style.setProperty('--glyph-y', `${-padY}px`);
  layer.style.setProperty('--glyph-w', `${viewW + 2 * padX}px`);
  layer.style.setProperty('--glyph-h', `${viewH + 2 * padY}px`);
}

/**
 * @param {HTMLCanvasElement} canvas
 * @param {HTMLElement} card
 * @param {ReturnType<typeof resolveHeroGlyphOptions>} opts
 * @param {string} text
 * @param {string} fillColor
 * @param {number} [offsetX=0]
 * @param {{ box?: { width: number, height: number } | null, offsetY?: number, minVisibleRatio?: number, glyphRatio?: number }} [layout]
 */
export function renderHeroGlyphCanvas(
  canvas,
  card,
  opts,
  text,
  fillColor,
  offsetX = 0,
  layout = {}
) {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;

  const dpr = window.devicePixelRatio || 1;
  const box = layout.box;
  const w = Math.max(1, box?.width ?? card.clientWidth);
  const h = Math.max(1, box?.height ?? card.clientHeight);
  const layer =
    canvas.closest('.post-card__glyph-layer') ||
    card.querySelector(':scope > .post-card__glyph-layer');

  canvas.style.display = 'block';
  canvas.style.opacity = '1';
  canvas.style.left = '';
  canvas.style.top = '';
  canvas.style.width = '';
  canvas.style.height = '';

  // Temporary card-sized surface to measure + place in design coords.
  canvas.width = Math.round(w * dpr);
  canvas.height = Math.round(h * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, w, h);

  const titleEl = card.querySelector('.post-title');
  const titleStyle = titleEl ? getComputedStyle(titleEl) : getComputedStyle(card);
  const widthRatio = Math.min(1.2, Math.max(0.4, opts.sizeRatio));
  const targetW = w * widthRatio;
  const minVisibleRatio = layout.minVisibleRatio ?? opts.minVisibleRatio;
  let fontSize = fitHeroGlyphFontSize(
    ctx,
    text,
    targetW,
    titleStyle,
    Math.round(targetW * 1.4)
  );

  ctx.globalCompositeOperation = 'source-over';
  ctx.fillStyle = fillColor;
  applyHeroGlyphFont(ctx, fontSize, titleStyle);
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  const offsetY = layout.offsetY ?? 0;
  let placement = constrainHeroGlyphPlacement(
    w,
    h,
    w / 2 + offsetX,
    h / 2 + offsetY,
    ctx.measureText(text),
    minVisibleRatio
  );
  if (placement.scale < 1) {
    fontSize = Math.max(12, Math.floor(fontSize * placement.scale));
    applyHeroGlyphFont(ctx, fontSize, titleStyle);
    placement = constrainHeroGlyphPlacement(
      w,
      h,
      w / 2 + offsetX,
      h / 2 + offsetY,
      ctx.measureText(text),
      minVisibleRatio
    );
  }

  const metrics = ctx.measureText(text);
  const left = Math.max(0, Number(metrics.actualBoundingBoxLeft) || metrics.width / 2);
  const right = Math.max(0, Number(metrics.actualBoundingBoxRight) || metrics.width / 2);
  const ascent = Math.max(0, Number(metrics.actualBoundingBoxAscent) || metrics.width / 2);
  const descent = Math.max(0, Number(metrics.actualBoundingBoxDescent) || 0);
  const basePad = resolveGlyphCanvasOverscan(w, h, {
    sizeRatio: opts.sizeRatio,
    offsetXRatioMax: Math.max(
      Math.abs(opts.offsetXRatioMin || 0),
      Math.abs(opts.offsetXRatioMax || 0)
    ),
    glyphRatio: layout.glyphRatio
  });
  const { padX, padY } = expandOverscanForInk(w, h, basePad, {
    left: placement.x - left,
    right: placement.x + right,
    top: placement.y - ascent,
    bottom: placement.y + descent
  });

  const canvasW = w + 2 * padX;
  const canvasH = h + 2 * padY;
  applyHeroCanvasBox(layer instanceof HTMLElement ? layer : null, padX, padY, w, h);

  canvas.width = Math.round(canvasW * dpr);
  canvas.height = Math.round(canvasH * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, canvasW, canvasH);
  ctx.globalCompositeOperation = 'source-over';
  ctx.fillStyle = fillColor;
  applyHeroGlyphFont(ctx, fontSize, titleStyle);
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.globalAlpha = 1;
  ctx.fillText(text, placement.x + padX, placement.y + padY);
}

/**
 * @param {ReturnType<typeof resolveHeroGlyphConfig>} heroCfg
 * @param {HTMLElement} card
 * @param {() => number} layoutRand
 * @param {() => number} colorRand
 */
export function resolveHeroGlyphPaintState(card, heroCfg, layoutRand, colorRand) {
  return {
    offsetX: pickHeroGlyphOffsetX(heroCfg, card.clientWidth, layoutRand),
    fillColor: resolveHeroGlyphPaint(card, heroCfg, colorRand)
  };
}

/**
 * Shift a full-card hero so its center sits in an empty band (patterns use the band as canvas; heroes stay mega).
 * @param {HTMLElement} card
 * @param {{ x: number, y: number, width: number, height: number }} region
 */
export function heroEmptySpaceOffsets(card, region) {
  const w = Math.max(1, card.clientWidth);
  const h = Math.max(1, card.clientHeight);
  return {
    offsetX: region.x + region.width / 2 - w / 2,
    offsetY: region.y + region.height / 2 - h / 2
  };
}

function renderHeroInEmptySpace(
  canvas,
  card,
  layer,
  heroCfg,
  text,
  fillColor,
  emptyRegion,
  extras = {}
) {
  const { offsetX, offsetY } = heroEmptySpaceOffsets(card, emptyRegion);
  layer.dataset.glyphEmptySpace = '1';
  renderHeroGlyphCanvas(canvas, card, heroCfg, text, fillColor, offsetX, {
    offsetY,
    glyphRatio: extras.glyphRatio
  });
}

/**
 * @param {HTMLElement} card
 * @param {HTMLElement} layer
 * @param {HTMLCanvasElement} canvas
 * @param {ReturnType<typeof resolveHeroGlyphConfig>} heroCfg
 * @param {string} text
 * @param {() => number} layoutRand
 * @param {() => number} colorRand
 * @param {{ x: number, y: number, width: number, height: number } | null} [emptyRegion]
 * @param {{ glyphRatio?: number }} [extras]
 */
export function applyHeroGlyph(
  card,
  layer,
  canvas,
  heroCfg,
  text,
  layoutRand,
  colorRand,
  emptyRegion = null,
  extras = {}
) {
  layer.classList.add('post-card__glyph-layer--hero');
  card.classList.add('post-card--hero-glyph');
  layer.dataset.heroGlyphText = text;
  const { offsetX, fillColor } = resolveHeroGlyphPaintState(card, heroCfg, layoutRand, colorRand);
  layer.dataset.glyphColor = fillColor;
  if (emptyRegion) {
    renderHeroInEmptySpace(
      canvas,
      card,
      layer,
      heroCfg,
      text,
      fillColor,
      emptyRegion,
      extras
    );
    return;
  }
  delete layer.dataset.glyphEmptySpace;
  renderHeroGlyphCanvas(canvas, card, heroCfg, text, fillColor, offsetX, {
    glyphRatio: extras.glyphRatio
  });
}

/**
 * @param {HTMLElement} card
 * @param {ReturnType<typeof resolveHeroGlyphConfig>} heroCfg
 * @param {((card: HTMLElement, slug: string) => { x: number, y: number, width: number, height: number } | null) | null} [resolveEmptyRegion]
 * @param {{ glyphRatio?: number }} [extras]
 */
export function refitHeroGlyph(card, heroCfg, resolveEmptyRegion = null, extras = {}) {
  if (!card.classList.contains('post-card--hero-glyph')) return;
  const layer = card.querySelector('.post-card__glyph-layer');
  const canvas = card.querySelector('[data-glyph-canvas]');
  const text = layer instanceof HTMLElement ? layer.dataset.heroGlyphText : '';
  if (!(layer instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) || !text) return;
  const slug = card.dataset.slug || text;
  const layoutRand = posterRandFromSlug(slug);
  const colorRand = posterRandFromSlug(`${slug}:hero-color`);
  const { offsetX, fillColor } = resolveHeroGlyphPaintState(card, heroCfg, layoutRand, colorRand);
  layer.dataset.glyphColor = fillColor;
  if (layer.dataset.glyphEmptySpace === '1' && resolveEmptyRegion) {
    const emptyRegion = resolveEmptyRegion(card, slug);
    if (emptyRegion) {
      renderHeroInEmptySpace(
        canvas,
        card,
        layer,
        heroCfg,
        text,
        fillColor,
        emptyRegion,
        extras
      );
      return;
    }
  }
  delete layer.dataset.glyphEmptySpace;
  renderHeroGlyphCanvas(canvas, card, heroCfg, text, fillColor, offsetX, {
    glyphRatio: extras.glyphRatio
  });
}

/** @param {HTMLElement} card @param {HTMLElement} layer */
export function clearHeroGlyph(card, layer) {
  layer.classList.remove('post-card__glyph-layer--hero');
  card.classList.remove('post-card--hero-glyph');
  delete layer.dataset.heroGlyphText;
  delete layer.dataset.glyphColor;
  delete layer.dataset.glyphBlendMode;
  delete layer.dataset.glyphEmptySpace;
  layer.style.removeProperty('--glyph-x');
  layer.style.removeProperty('--glyph-y');
  layer.style.removeProperty('--glyph-w');
  layer.style.removeProperty('--glyph-h');
  const canvas = layer.querySelector('[data-glyph-canvas]');
  if (canvas instanceof HTMLCanvasElement) {
    const ctx = canvas.getContext('2d');
    ctx?.clearRect(0, 0, canvas.width, canvas.height);
    canvas.style.display = 'none';
    canvas.style.opacity = '';
    canvas.style.left = '';
    canvas.style.top = '';
    canvas.style.width = '';
    canvas.style.height = '';
  }
}
