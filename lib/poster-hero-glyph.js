 ...
 * @param {{ box?: { width: number, height: number } | null, offsetY?: number, minVisibleRatio?: number }} [layout]
 */
export function renderHeroGlyphCanvas(
  ...
  if (placement.scale < 1) {
    applyHeroGlyphFont(ctx, Math.max(12, Math.floor(fontSize * placement.scale)), titleStyle);
...
function renderHeroInEmptySpace(canvas, card, layer, heroCfg, text, fillColor, emptyRegion) {
...
export function applyHeroGlyph(
  ...
  if (emptyRegion) {
    renderHeroInEmptySpace(canvas, card, layer, heroCfg, text, fillColor, emptyRegion);
    return;
  }
 ...
 */
export function refitHeroGlyph(card, heroCfg, resolveEmptyRegion = null) {
