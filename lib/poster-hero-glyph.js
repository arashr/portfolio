 ...
 *
 * Blend is baked on canvas (globalCompositeOperation): one composite per pattern
 * (avoids per-letter opacity stacking) and stable poster PDF export.
 */
...
export function resolveHeroGlyphOptions(cfg) {
  ...
  return {
    ...
    lengthMax,
    color: String(
      nested.color ?? nested.glyphColor ?? nested.titleColor ?? HERO_GLYPH_DEFAULTS.color
    ),
...
export function resolveHeroGlyphPaint(card, opts) {
  const key = String(opts.color ?? opts.glyphColor);
...
function renderHeroInEmptySpace(canvas, card, layer, heroCfg, text, blendMode, opacity, emptyRegion) {
...
export function applyHeroGlyph(
  ...
  const { opacity } = resolveHeroGlyphPaintState(
    card,
    heroCfg,
    blendMode,
    layoutRand,
  ...
  if (emptyRegion) {
    renderHeroInEmptySpace(canvas, card, layer, heroCfg, text, blendMode, opacity, emptyRegion);
    return;
  }
  ...
  layer.style.removeProperty('--glyph-w');
  layer.style.removeProperty('--glyph-h');
  const { offsetX } = resolveHeroGlyphPaintState(card, heroCfg, blendMode, layoutRand, opacityRand);
  renderHeroGlyphCanvas(canvas, card, heroCfg, text, blendMode, offsetX, opacity);
}
