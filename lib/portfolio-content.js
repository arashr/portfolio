...
export async function resolveCaseStudyItems(
  ...
  const items = await Promise.all(
    index.cases.map(async (path, index) => {
      ...
      const { title, subtext, stats, credit, coverGroundKey } = peekCaseStudyListing(
        text,
        filename
      );
      const slug = path.replace(/\.[^.]+$/, '').split('/').pop() || 'case';
      // A5-COVER-GROUND: pass through listing ground for home / more-cases color only.
      return { path, slug, title, subtext, stats, credit, coverGroundKey, index };
    })
  );
  return items;
}
