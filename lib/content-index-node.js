...
import { buildAssetDimensionsMap, scanContentImageAssets } from './image-dimensions-node.js';
...
export function contentRevision(
  ...
  extraPaths = [SITE_CONFIG_PATH, HOME_ASIDE_PATH]
) {
  const parts = [];
  ...
  for (const rel of scanContentImageAssets(CONTENT_DIR, repoRoot)) {
    const full = join(repoRoot, rel);
    try {
      const stat = statSync(full);
      parts.push(`${rel}:${stat.mtimeMs}:${stat.size}`);
...
export function buildContentIndexPayload(contentDir = CONTENT_DIR, repoRoot = process.cwd()) {
  ...
  return {
    ...
    cases,
    assetDimensions: buildAssetDimensionsMap(contentDir, repoRoot)
  };
}
